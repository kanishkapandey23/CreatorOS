...
import { getLastStudio } from '@/lib/studio-persistence';
...
export default function WorkspacePage() {
  ...
  const lastStudio = getLastStudio();
  const resumeStory = (data?.recentStories || []).find((s) => s.id === lastStudio?.storyId);
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-5 py-10 md:px-8 md:py-14">
      ...
      {lastStudio?.path && (
        <motion.section
          initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ ...easing, delay: 0.03 }}
          className="mt-8"
        >
            ...
            <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
              <div>
                <div className="inline-flex items-center gap-2 rounded-full bg-secondary px-2.5 py-1 text-[11px] font-medium text-ink-muted">
                  <PenLine className="h-3 w-3 text-brand" /> Resume writing
                </div>
                <h2 className="mt-3 font-display text-[22px] font-semibold leading-snug text-ink md:text-[24px]">
                  {resumeStory?.title || 'Your last workspace'}
                </h2>
                <p className="mt-2 text-[13px] text-ink-muted">Continue where you left off in Studio.</p>
              </div>
              <Button asChild className="h-11 rounded-xl bg-ink px-5 text-[13.5px] text-white hover:bg-ink/90">
                <Link href={lastStudio.path}>Open Studio<ArrowRight className="ml-2 h-4 w-4" /></Link>
              </Button>
            </div>
          </div>
        </motion.section>
          ...
          <div className="relative flex flex-col items-start justify-between gap-5 md:flex-row md:items-center">
            <div className="max-w-2xl">
              ...
              <h2 className="mt-3 font-display text-[22px] font-semibold leading-snug text-ink md:text-[26px]">
                {data?.continueReflection?.title || 'What did you change your mind about this week?'}
              </h2>
              <div className="mt-4 flex items-center gap-3 text-[12.5px] text-ink-muted">
                <div className="h-1.5 w-40 overflow-hidden rounded-full bg-secondary">
                  <div
                    ...
                    style={{ width: `${Math.min(100, ((data?.continueReflection?.progress || 0) / (data?.continueReflection?.total || 4)) * 100)}%` }}
                  />
                </div>
                {data?.continueReflection?.progress || 0} of {data?.continueReflection?.total || 4} prompts
              </div>
            </div>
            <Button asChild className="h-11 rounded-xl bg-ink px-5 text-[13.5px] text-white hover:bg-ink/90">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {(data?.recentStories || []).map((s) => (
              <Link
                key={s.id}
                href={`/stories/${s.id}`}
                className="card-elev group p-5 transition-shadow hover:shadow-pop"
              >
      ...
      <section className="mt-10">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            { t: 'Capture a story', d: 'Start a reflection session.', icon: Plus, action: handleNewStory },
            ...
            a.href ? (
              ...
              <button key={a.t} onClick={a.action} className="card-elev flex items-start gap-3 p-4 text-left transition-colors hover:bg-secondary/40">
                <div className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-secondary text-ink">
                  <a.icon className="h-[18px] w-[18px]" />
                </div>
                <div>
